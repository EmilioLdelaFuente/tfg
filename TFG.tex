...
Normalmente este control se apliccaba a nivel individual en cada aerogenerador. En España, este planteamiento era v\'alido para cumplir el RD 2818, que solo tomaba en cuenta el alejamiento del factor de potencia unitario con los datos mensuales. Sin embargo la noramtiva actual requiere un ajuste fino que solo se puede conseguir añadiendo una capa de control global regulando el parque al completo. \par

Estos controladores de planta, conocidos como \emph{Power Plant Controller} o PPC, cumple otras funciones a parte de cumplir con las restricciones de las normativas. Son los encargados de maximizar la producci\'on, seguir las instrucciones del operador de red o cumplir ciertas pautas que indique el gestor del parque. \par
...
La red esta representada como una fuente de tensi\'on y una impedancia de car\'acter indiuctivo. Se ha utilizado el bloque de fuente de tensi\'on variable para representar las inestabilidades en la red. Es la parte más simple del modelo el\'ectrico en cuanto a n\'umero de elementos, solo se necesitan dos bloques para representarla como se ve en la figura \ref{GridSimulink}. \par
...
Es la red la que determina la consigna del control de reactiva, porque esta depende de la tensi\'on de red como se vio en la secci\'on de los c\'odigos de red. Pero no solo los cambios que se producen en la tensi\'on de la red no solo se producen por las variaciones programadas en la fuente variable, la parte inductiva de la red provoca un aumento en la potencia reactiva que consume cuando la intensidad que suministra la planta aumenta.  \par

Por lo tanto el aumento de la potencia activa que suminsitra la red, por esta impedancia inductiva, provoca una desviaci\'on natural en la tensi\'on del PCC. Esta desviaci\'on de tensi\'on sera m\'as significativa mientras mayor sea la potencia que la planta genera. \par
...
En el caso de que la planta no esta aplicando la compensaci\'on de reactiva la consigna de esta misma sera 0. La consigna tambien se establece como cero cuando se quiere que la planta aporte la reactiva natural provocada por su propio car\'acter. \par
...
En la figura \ref{GC1} se muestra la evoluci\'on temporal potencia reactiva de la red, $Grid\_Q$, y de la consigna de esta misma, $Q_{SP}$. Todas las medidas de potencia estan en Watios y la de tiempo en segundos.\par

%Añadir la grafica de los resultados para 0 W

Tanto en este escenario como en el segundo no se realizara el ensayo de no aplicar compesnaci\'on del c\'odigo de red, porque el propio c\'odigo de red nos impedia aplicar la compensaci\'on. \par

	\paragraph{Segundo escenario}

Al igual que en el primer caso la potencia nominal es de $0W$ pero en planta capacitiva. La limitaci\'on sobre la potencia reactiva impuesta por la potencia activa tiene el mismo.
...
%Añadir las graficas de los resultados de 1MWL

En la figura \ref{L1MWgc} se puede ver la porcentaje de potencia reactiva respecto de activa en el eje y frente a la tensi\'on en cientos de kV. El resultado mostrado en concreto son de la prueba para una perturbaci\'on sinusoidal, en la que se puede apreciar que durante toda la perturbaci\'on el comportamiento de la planta estuvo dentro de los limites del c\'odigo de red. \par
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.5\textwidth]{/Casos/L1MWgc.PNG}
...
En la figura \ref{GC4} se ve la respuesta ante las diferentes perturbaciones de la red antes comentadas. \par

%Figura de perturbaciones de tension
...
%escribir porque sale capacitivo :P SUERTE CON ESO jaja Salu2
...
En la figura \ref{GC6} se ven los resultados del primer ensayo.  \par

%Si no he puesto una gráfica GC en mi vida Hulio
